...
/// struct for typed errors of method [`get_instruments_instruments_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetInstrumentsInstrumentsGetError {
...
pub async fn get_instruments_instruments_get(configuration: &configuration::Configuration, isin: Option<Vec<String>>, mic: Option<Vec<String>>, search: Option<&str>, _type: Option<Vec<crate::market_data::models::ModelType>>, currency: Option<Vec<String>>, tradable: Option<bool>, sorting: Option<crate::market_data::models::SortType>, limit: Option<i32>, page: Option<i32>, authorization: Option<&str>, jwt: Option<&str>, x_consumer_custom_id: Option<&str>) -> Result<crate::market_data::models::PageInstrument, Error<GetInstrumentsInstrumentsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/instruments/", local_var_configuration.base_path);
    ...
    if let Some(ref local_var_str) = mic {
        local_var_req_builder = match "multi" {
            "multi" => local_var_req_builder.query(&local_var_str.into_iter().map(|p| ("mic".to_owned(), p.to_string())).collect::<Vec<(std::string::String, std::string::String)>>()),
            _ => local_var_req_builder.query(&[("mic", &local_var_str.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]),
        };
    }
    ...
    if let Some(ref local_var_str) = currency {
        local_var_req_builder = match "multi" {
            "multi" => local_var_req_builder.query(&local_var_str.into_iter().map(|p| ("currency".to_owned(), p.to_string())).collect::<Vec<(std::string::String, std::string::String)>>()),
            _ => local_var_req_builder.query(&[("currency", &local_var_str.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]),
        };
    }
    if let Some(ref local_var_str) = tradable {
        local_var_req_builder = local_var_req_builder.query(&[("tradable", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetInstrumentsInstrumentsGetError> = serde_json::from_str(&local_var_content).ok();
